...
- **Localized UI** — English, Español, Français, Português (Brasil) and Русский. Follows the browser's UI language by default (every regional variant maps to its base locale, so `es-CR`/`es-419`/`es-MX` all get Español and `fr-CA`/`fr-BE` get Français), with a manual override in Settings that applies immediately across the popup, approval window and background. The same `_locales/` files drive the Chrome Web Store listing and extension name via the manifest's `__MSG_*__` placeholders, so there is one source of truth per string.
...
_locales/           en · es · fr · pt_BR · ru message catalogs (also the store listing)
